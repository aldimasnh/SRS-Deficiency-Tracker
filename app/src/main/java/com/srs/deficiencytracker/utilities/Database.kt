@file:Suppress("SpellCheckingInspection")

package com.srs.deficiencytracker.utilities

@Suppress("unused")
object Database {
    const val mp = "https://mobilepro.srs-ssms.com/"
    const val server = "https://palmsentry.srs-ssms.com/"
    const val TAG_SUCCESS = "success"
    const val TAG_MESSAGE = "message"

    const val blokReg1 = "Jqayb4aORkQvs2oEa.KdQ"

    const val TAG_USERNAME = "username"
    const val TAG_PASSWORD = "password"
    const val TAG_USERID = "user_id"

    const val TAG_NAMA = "nama_lengkap"
    const val TAG_DEPARTEMEN = "departemen"
    const val TAG_LOKASIKERJA = "lokasi_kerja"
    const val TAG_JABATAN = "jabatan"
    const val TAG_NOHP = "no_hp"
    const val TAG_EMAIL = "email"
    const val TAG_LOKASI = "lokasi_kerja"
    const val TAG_AKSES = "akses_level"
    const val TAG_CONFIRMPASS = "confirm_password"
    const val TAG_AFDELING = "afdeling"

    var blokEst: Array<String> = arrayOf("SLE", "KNE", "RDE", "PLE", "KDE", "BKE", "RGE", "SGE", "UPE", "SYE", "NBE", "BGE", "SRE", "SKE", "LDE", "PDE",
        "NKE", "MRE", "SPE", "BTE", "SBE", "NNE", "MLE", "SCE", "KTE", "BDE", "MKE", "BSE", "BHE", "GDE", "PKE", "BWE")
}